 ...
 */
    ...
    .controller('modalLoginController', ['$scope', '$log', '$modalInstance', '$modalStack', 'authenticateUser', 'userService', '$state', '$stateParams', '$modal', '$rootScope',
        function ($scope, $log, $modalInstance, $modalStack, authenticateUser, userService, $state, $stateParams, $modal, $rootScope) {
                ...
                authenticateUser.authenticate(loginInfo).then(
                    function (response) {
                        ...
                        if (response.status === 701) {
                            ...
                            $rootScope.$broadcast('NeedChangePassword', {status : response.status});
    ...
    .controller('modalLoginChoiceController', ['$scope', '$log', '$modalInstance', 'authenticateUser', 'userService', '$state', '$stateParams', '$modal', '$rootScope','$window',
        function ($scope, $log, $modalInstance, authenticateUser, userService, $state, $stateParams, $modal, $rootScope, $window) {
            ...
            $scope.openPassword = function() {
              ...
              $state.go("loginpanel", $stateParams);
            };
    ...
    .controller('loginChoiceController', ['$scope', '$log', '$state', '$stateParams', 'userService',
        function ($scope, $log, $state, $stateParams, userService) {
    ...
    .service('loginChoicePanel', ['$window', '$modal', '$log',
    ...
    .controller('ecaslogincontroller', ['$scope', '$log', '$modalInstance', 'authenticateUser', 'userService', '$state', '$stateParams', '$window','ENVCONFIG',
        function ($scope, $log, $modalInstance, authenticateUser, userService, $state, $stateParams, $window,ENVCONFIG) {
            var callback = $window.document.URL.split('#')[0] + '#/jwtcallback';
            var _url = "/usm-administration/rest/ping?jwtcallback=" + callback;
            ...
            if(_.isArray(ENVCONFIG.rebaseApiList)){
              ...
              for (var i = 0, leni = ENVCONFIG.rebaseApiList.length; i < leni; i++) {
                  var rebaseApi = ENVCONFIG.rebaseApiList[i];
                  for (var j = 0, lenj = rebaseApi.urlPatterns.length; j < lenj; j++) {
                      var pattern = rebaseApi.urlPatterns[j];
                      if (_url.indexOf(pattern) === 0 && (_.isUndefined(rebaseApi.domain) || window.location.hostname.indexOf(rebaseApi.domain)>=0)) {
                          isRebaseApiRequest = true;
                          break;
                      }
                  }
                  if (isRebaseApiRequest) {
                      var newUrl = rebaseApi.apiBase + _url;
                      $log.debug("Rebase request from " + _url + " to " + newUrl);
                      _url = newUrl;
                      break;
                  }
              }
