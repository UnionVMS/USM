...
/**
 * Unit-test for the ApplicationJpaDao 
 */
...
public class ApplicationJpaDaoTest extends DeploymentFactory {
	...
	private static void assertContainsAllApps(List<ApplicationEntity> apps) {
		Assert.assertEquals(APPLICATION_NAMES.length, apps.size());
		...
		Assert.assertTrue(names.containsAll(Arrays.asList(APPLICATION_NAMES)));
	}
	...
	private static void assertChildNamePointsParent(ApplicationEntity app,
			ApplicationEntity child) {
		Assert.assertTrue(child.getName().startsWith(app.getName()));
	}

	private static void assertParent(ApplicationEntity app) {
		if (hasParentAccordingName(app)) {
			Assert.assertEquals(app.getParentApplication().getName(), parentNameAccordingName(app));
		} 
	}
