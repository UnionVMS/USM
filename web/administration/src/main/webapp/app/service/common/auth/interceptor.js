 ...
 *
 * @description
 * `authInterceptor` is responsible for adding the authorisation headers to http requests made to the backend.
 */
...
//			};
//			var ecasTransform = function(data, headers, status) {
//					/*
//					 * If we get a success code 200 but an ECAS_AUTHENTICATION_REQUIRED status we can update the code to 403
//					 */
//					 if (data && data.status && (data.status === "ECAS_AUTHENTICATION_REQUIRED" || data.status === "CAS_AUTHENTICATION_REQUIRED")) {
//							 _log.debug('Interceptor ECAS/CAS AUth needed : ' + data);
//							 _log.debug('Interceptor rejecting response: headers' + headers);
//							 status = "403";
//						 }
//			  return data;
//			};
...
//                    request.transformResponse = appendTransform(request.transformResponse, ecasTransform);
                    ...
                    */
                    ...
                    for (var i = 0, leni = config.rebaseApiList.length; i < leni; i++) {
                        ...
                        for (var j = 0, lenj = rebaseApi.urlPatterns.length; j < lenj; j++) {
                            ...
                            if (request.url.indexOf(pattern) === 0 && (_.isUndefined(rebaseApi.domain) || window.location.hostname.indexOf(rebaseApi.domain)>=0)) {
				...
				response: function (response) {
					...
					if (!_.isUndefined(response.headers()["extstatus"])) {
						...
						if (response.headers()["extstatus"] === "701") {
							_log.debug("User authenticated but password expired (701). User should change password NOW!");
							$rootScope.$broadcast('NeedChangePassword', {status : response.headers()["extstatus"]});
							return $q.reject(response);
						} else if (response.headers()["extstatus"] === "773") {
							_log.debug("User authenticated but password is about to expire (773). Should suggest to change password.");
					...
					if (response.data && response.data.status && (response.data.status === "ECAS_AUTHENTICATION_REQUIRED" || response.data.status === "CAS_AUTHENTICATION_REQUIRED")) {
						_log.debug('Interceptor ECAS/CAS AUth needed : ' + response.data);
						response.status = "403";
						response.statusText = response.data.status;
						return $q.reject(response);
					}
				...
				responseError: function (rejection) {
                    ...
                    _log.debug("Interceptor handling rejected request");
