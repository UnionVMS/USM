...
sharedModule.controller('MenuCtrl', ['$rootScope', '$scope', '$state', '$translate', '$stateParams', '$log', '$cookies', 'userService', 'changesService',
    function ($rootScope, $scope, $state, $translate, $stateParams, $log, $cookies, userService, changesService) {
        ...
        $scope.isAllowed = userService.isAllowed;
        $scope.manageUsers = userService.isAllowed;
...
sharedModule.controller('userMenuController', ['$log', 'selectContextPanel', 'authRouter', '$scope', 'userService', 'renewloginpanel', '$rootScope', '$state', 'policiesService', 'ENVCONFIG',
    function ($log, selectContextPanel, authRouter, $scope, userService, renewloginpanel, $rootScope, $state, policiesService,ENVCONFIG) {
        ...
        var init = function () {
            ...
            $scope.ECAS = ENVCONFIG.idm==="ECAS";
            checkLdapEnabled();
        };
        init();

        // we could be on a state that does not require us to be logged in.
        // but let's call the findSelectedContext promise and init again upon return
