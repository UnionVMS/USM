...
//                }, 15000);
                    ...
                    _backendPing().then(
                        function (response) {
                            ...
                            if (response.data && response.data.status && (response.data.status === "ECAS_AUTHENTICATION_REQUIRED" || response.data.status === "CAS_AUTHENTICATION_REQUIRED")) {
                              ...
                              if (response.status && response.status + "" === "200" && !_.isUndefined(response.headers()["authorization"])) {
                                  $log.debug('Resolving _backendPing with "Authenticated", response.status: ' + response.status);
                                  // we have a successful ping but we did not have a valid token to start with
                                  // use the returned token to login
            ...
            var _findContexts = function () {
                ...
                _isUserIdentifiedPromise().then(
                    function (user) {
                        $log.debug('_isUserIdentifiedPromise success response (1): '+ user);
