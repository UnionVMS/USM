...
public class ManageUserServiceBean implements ManageUserService {
    ...
    public UserAccount createUser(ServiceRequest<UserAccount> request) {
        ...
        try {
            auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), AuditOperationEnum.CREATE.getValue(), AuditObjectTypeEnum.USER.getValue() + " " + request.getBody().getUserName(), request.getBody().getNotes(), request.getRequester());
    ...
    public UserAccount updateUser(ServiceRequest<UserAccount> request) {
        ...
        String auditLog = null;
        try {
            auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), AuditOperationEnum.UPDATE.getValue(), AuditObjectTypeEnum.USER.getValue() + " " + request.getBody().getUserName(), request.getBody().getNotes(), request.getRequester());
        } catch (AuditModelMarshallException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
        auditProducer.sendModuleMessage(auditLog);
    ...
    public void changePassword(ServiceRequest<ChangePassword> request)
        ...
        String auditLog = null;
        try {
            auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), AuditOperationEnum.UPDATE.getValue(), AuditObjectTypeEnum.PASSWORD.getValue() + " " + request.getBody().getUserName(), request.getRequester());
        } catch (AuditModelMarshallException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
        auditProducer.sendModuleMessage(auditLog);
    ...
    private void auditAction(String actionName, ServiceRequest<UserAccount> request) {
        ...
        String auditLog = null;
        try {
            auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), actionName, "ManageUserService " + request.getBody().getUserName(), request.getRequester(), request.getBody().getNotes());
        } catch (AuditModelMarshallException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
        auditProducer.sendModuleMessage(auditLog);
    ...
    public ChallengeInformationResponse setChallengeInformation(ServiceRequest<ChallengeInformationResponse> request, String userName)
        ...
        for (int i = 0; i < challengeInformations.size(); i++) {
            ...
            String auditLog = null;
            try {
                auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), auditOperation, AuditObjectTypeEnum.CHALLENGE.getValue() + " " + userName, userName, request.getRequester());
            } catch (AuditModelMarshallException e) {
                throw new RuntimeException(e.getMessage(), e);
            }
            auditProducer.sendModuleMessage(auditLog);
    ...
    public void resetPassword(ServiceRequest<ResetPasswordQuery> request)
        ...
        String auditLog = null;
        try {
            auditLog = AuditLogMapper.mapToAuditLog(USMApplication.USM.name(), AuditOperationEnum.RESET.getValue(), AuditObjectTypeEnum.PASSWORD.getValue() + " " + userName, userName, request.getRequester());
        } catch (AuditModelMarshallException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
        auditProducer.sendModuleMessage(auditLog);
