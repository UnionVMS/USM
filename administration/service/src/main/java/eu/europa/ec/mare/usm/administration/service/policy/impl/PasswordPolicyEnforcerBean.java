...
public class PasswordPolicyEnforcerBean implements PasswordPolicyEnforcer {
    ...
    private void checkLength(Properties properties, String password) {
        int minLength = getInt(properties, MIN_LENGTH, 0);

        LOGGER.debug(MIN_LENGTH + ": " + minLength);

        if (minLength != 0) {
            if (password.length() < minLength) {
                throw new IllegalArgumentException(MUST_CONTAIN + minLength + " characters");
            }
        }
    }
    ...
    private void checkDigits(Properties properties, String password) {
        ...
        if (minDigits != 0) {
            ...
            if (digits < minDigits) {
                throw new IllegalArgumentException(MUST_CONTAIN + minDigits + " digits");
            }
        }
    }
    ...
    private void checkSpecial(Properties properties, String password) {
        ...
        if (minSpecial != 0) {
            ...
            if (special < minSpecial) {
                throw new IllegalArgumentException(MUST_CONTAIN + minSpecial + " special characters");
            }
        }
    }
    ...
    private void checkMixUpperLowerCase(Properties properties, String password) {
        ...
        if (mixUpperLowercase) {
            ...
            if (uppercase < 1 || lowercase < 1) {
                throw new IllegalArgumentException(MUST_CONTAIN + "one uppercase and one lowercase characters");
            }
        }
    }
            ...
            if (t != null) {
                try {
                    ...
                    LOGGER.warn("Property value for '" + key +
                            "' is not an integer number. Using default value: " + defaultValue);
                }
            }
        }
        return ret;
        ...
        if (properties != null) {
            ...
            if (t != null) {
                try {
                    ...
                    LOGGER.warn("Property value for '" + key + "' is not boolean. Using default value: " + defaultValue);
                }
            }
        }
        return ret;
